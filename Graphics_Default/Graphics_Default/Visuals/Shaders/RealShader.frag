...
void main()
{
	...
	vec3 NewNormal = CalculateNormal();

	vec3 result;
	for(int i = 0; i < NUMBER_OF_LIGHTS; i++)
	{
		if(!lights[i].render) continue; //Light is not currently active do not render it.
		
		if(lights[i].type == Directional) {result += AddDirectional(lights[i], testNormal, fragPos, viewDir);}
		else if(lights[i].type == Spotlight){AddSpotlight(lights[i], NewNormal, fragPos, viewDir);}
		else {result += AddPointLight(lights[i], NewNormal, fragPos, viewDir);}
	}
