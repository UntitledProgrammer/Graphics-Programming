...
private:
	//Attributes:
	const SDL_Keycode forward = SDLK_w, backward = SDLK_s, left = SDLK_a, right = SDLK_d, toggle = SDLK_LSHIFT;
...
public:
	...
	virtual void Update()
	{
		...
		body->transform.position += glm::vec3(INPUT->getAxis(right, left) * speed, 0, INPUT->getAxis(forward, backward) * speed);
