...
void Simulated::DrawCentre(float scale)
{
	...
	//Draw:
	glBegin(GL_LINES);
	glm::vec3 p1 = glm::vec3(transform.GetMatrix()[0][3], transform.GetMatrix()[1][3], transform.GetMatrix()[2][3]), p2 = p1;

	//Red line.
	glColor3f(1, 0, 0); //Red. (0.0f - 1.0f : 0 - 255).
	glVertex3fv(&p1.x);
	p2 = p1 + glm::vec3(scale, 0, 0);
